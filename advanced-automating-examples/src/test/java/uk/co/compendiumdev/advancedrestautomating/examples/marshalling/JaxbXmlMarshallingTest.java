 ...
 */
public class JaxbXmlMarshallingTest {
    ...
    private static class AListsCollectionResponse{
     ...
     public void createListWithXML(){
         ...
         // remove the BOM character to allow JAXB processing
         String fixOutput = response.body().asString().replace("\uFEFF", "");

         AListsCollectionResponse lists = JAXB.unmarshal(new StringReader(fixOutput),
         ...
         String fixedOutput = amendedListsResponse.body().asString().replace("\uFEFF", "");
         final AListsCollectionResponse amendedLists = JAXB.unmarshal(new StringReader(fixedOutput),
                 AListsCollectionResponse.class);
